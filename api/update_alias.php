...
$stmt->bind_param('ii',$account_id,$uid); $stmt->execute();
if (!$stmt->get_result()->fetch_assoc()){ header('Location: ../user/accounts.php?error=forbidden'); exit; }

// alias único global
$stmt = $mysqli->prepare("SELECT id FROM accounts WHERE alias=? AND id<>? LIMIT 1");
$stmt->bind_param('si',$alias,$account_id); $stmt->execute();
if ($stmt->get_result()->fetch_assoc()){ header('Location: ../user/accounts.php?error=dup_alias'); exit; }
